...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  {
    id: 'price-track',
    name: 'Rastreador de precios',
    desc: 'Registra el precio de algo que compras seguido para vigilarlo',
    icon: '🏷️',
    check: (s) => (s.priceItems ?? []).some((p) => p.history.length >= 1),
  },
  {
    id: 'price-hunter',
    name: 'Cazaofertas',
    desc: 'Registra 3 precios de un mismo producto para comparar y comprar barato',
    icon: '📉',
    check: (s) => (s.priceItems ?? []).some((p) => p.history.length >= 3),
  },
  {
    id: 'restock-hero',
    name: 'Reabastecido',
  ...
  // ── Tablero (lienzo infinito estilo Excalidraw): dibuja, anota y guarda ideas.
  {
    id: 'board-first',
    name: 'Lienzo estrenado',
    desc: 'Dibuja tu primer elemento en el Tablero (tu pizarra infinita)',
    icon: '🖊️',
    check: (s) => (s.boardScene?.elements?.length ?? 0) >= 1,
  },
  {
    id: 'board-draw',
    name: 'Trazo libre',
    desc: 'Haz un trazo a mano alzada en el Tablero',
    icon: '✏️',
    check: (s) => (s.boardScene?.elements ?? []).some((e: any) => e?.type === 'freedraw'),
  },
  {
    id: 'board-text',
    name: 'Apunta la idea',
    desc: 'Escribe un texto en el Tablero',
    icon: '🔤',
    check: (s) => (s.boardScene?.elements ?? []).some((e: any) => e?.type === 'text'),
  },
  {
    id: 'board-shape',
    name: 'Diagramador',
    desc: 'Dibuja una figura (rectángulo, círculo o diamante) en el Tablero',
    icon: '🔷',
    check: (s) =>
      (s.boardScene?.elements ?? []).some((e: any) =>
        ['rectangle', 'ellipse', 'diamond'].includes(e?.type)
      ),
  },
  {
    id: 'board-connect',
    name: 'Conectando ideas',
    desc: 'Traza una flecha para conectar cosas en el Tablero',
    icon: '➡️',
    check: (s) => (s.boardScene?.elements ?? []).some((e: any) => e?.type === 'arrow'),
  },
  {
    id: 'board-photo',
    name: 'Recuerdo pegado',
    desc: 'Inserta una imagen/recuerdo en el Tablero',
    icon: '📸',
    check: (s) => (s.boardScene?.elements ?? []).some((e: any) => e?.type === 'image'),
  },
  {
    id: 'board-full',
    name: 'Lienzo vivo',
    desc: 'Ten 25 elementos en tu Tablero a la vez',
    icon: '🗂️',
    check: (s) => (s.boardScene?.elements?.length ?? 0) >= 25,
  },
]
